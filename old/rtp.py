...
default_params = ('test.h264', 1199, 50, 0, 260, 0, 1, 0)
...
if N < 2:
    ...
    print('filename = ' + '"' + str(default_params[0]) + '"', 'b = ' + str(default_params[1]), 'a = ' + str(default_params[2]),\
          'mode = ' + str(default_params[3]), 'packetsize = ' + str(default_params[4]), 'pad = ' + str(default_params[5]) + '\n', sep = '\n')
        ...
        if byte is 0:
            if bytestream[index+1:index+4] == bytes((0, 0, 1)):
                ...
                if len(unit) > unitsize:
                    ...
                    if type1 in [1, 5]:
                        ts_numerator += ts_num_inc
                        print(timestamp, math.ceil(ts_numerator/b), ts_numerator, ts_num_inc/b)
                        timestamp = ts_start + math.ceil(ts_numerator/b)
                        curtime = time.perf_counter() - ref_point
                        if timescale - curtime > 0:
                            my_sleep(timescale - curtime)
            ...
            if bytestream[index+1:index+4] == bytes((0, 0, 1)):
                ...
                if len(unit) > unitsize:
                    ...
                    if paddingsize:
                        ...
                        packet = rtpheader + NALheader + unit[(N-1)*(unitsize-2)+1:] + bytes([0]*(paddingsize-1)) + bytes([paddingsize])
                    else:
                        rtpheader = makeheader(2, 0, 0, cc, 0, pt, sn, timestamp, ssrc, csrc)
                        packet = rtpheader + NALheader + unit[(N-1)*(unitsize-2)+1:]
                    sn += 1
